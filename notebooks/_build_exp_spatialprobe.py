...
c.append(nbf.v4.new_markdown_cell(
    "## How to read this\n"
    "- **Spatial readouts (grid at g=2/4, and especially peak-count) clearly "
    "exceed the pooled channel probe** = the rendered count is carried by the "
    "*spatial layout* of up-block self-attention, more than by any global "
    "summary. That is the positive demonstration the earlier (broken) probe "
    "could not give, and it supports the object-layout framing.\n"
    "- **peak-count $r^2$ highest** = the count is a *nonlinear, instance-"
    "multiplicity* property (a counting readout beats any linear probe) - "
    "consistent with a slot/layout code rather than a linear magnitude.\n"
    "- **All comparable / pooled not beaten** = the count is not specifically "
    "spatial in this readout; report honestly and lean on the causal patch "
    "results instead.\n"
    "- (Fixed vs the earlier version: raw energy instead of min-max saliency, and "
    "cross-validated ridge instead of a single unregularized split, which had "
    "produced spurious negative $R^2$.)"))
