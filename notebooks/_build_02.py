"""Generate notebooks/02_signal_trace.ipynb (Phase 2 — trace the count signal).

Probes count decodability across attention layers x denoising timesteps to
find WHERE and WHEN SDXL commits to a count, and whether the break is
image-side (attn1), matching (attn2), or text. Thin Colab driver over src/.

Run: python notebooks/_build_02.py
"""
...
c.append(nbf.v4.new_markdown_cell(
    ...
    "Phase 1 showed the TEXT drives the count but realizes it unreliably "
    "(over-generation). Here we find WHERE and WHEN the count is committed, "
    "and whether the break is **image-side** (self-attn `attn1`), the "
    "**matching** junction (cross-attn `attn2`), or **text**.\n\n"
    "For each image we capture pooled activations at curated sites at a few "
    "timesteps, then measure how decodable the **requested** vs **rendered** "
    "count is at each (site, step).\n\n**Runtime:** GPU."))
...
c.append(nbf.v4.new_markdown_cell(
    "## How to read this\n"
    "- **Requested-count R2 high on `attn2` but low on `attn1`** = the number "
    "is present only in the text/matching channel, not yet built image-side "
    "(the text-leak we must control for).\n"
    "- **Rendered-count R2 low early, rising at a specific (site, step)** = the "
    "**commitment / realization point** — where the eventual (often wrong) "
    "count gets written into the image. That site+step is the Phase-4 "
    "intervention target.\n"
    "- **Large positive gap (req >> ren)** = the model 'knows' the number there "
    "but hasn't realized it yet -> the break is downstream of that point.\n"
    "- **attn1 vs attn2 means (last cell):** if rendered count is carried mainly "
    "by `attn2` and only late, the failure lives in **matching/realization** "
    "(Phase-1-consistent); if `attn1` carries it early, the image commits "
    "early and the fix belongs upstream."))
...
import os
out = os.path.join(os.path.dirname(__file__), "02_signal_trace.ipynb")
nbf.write(nb, out)
print("wrote", out)
