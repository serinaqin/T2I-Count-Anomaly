...
c.append(nbf.v4.new_code_cell(
    ...
    "    # Manual decode (version-robust; avoids processor.post_process API drift):\n"
    "    inp = owl_proc(text=[[f'a photo of a {obj}']], images=img, return_tensors='pt').to(pipe.device)\n"
    "    out = owl_model(**inp)\n"
    "    scores = out.logits[0].sigmoid().squeeze(-1).cpu().numpy()   # (N,) per-box objectness\n"
    "    boxes = out.pred_boxes[0].cpu().numpy()                       # (N,4) cxcywh normalized\n"
    "    W, H = img.size; dets = []\n"
    "    for s, (cx, cy, w, h) in zip(scores, boxes):\n"
    "        if s < thr: continue\n"
    "        dets.append({'label': obj, 'score': float(s),\n"
    "                     'box': [(cx-w/2)*W, (cy-h/2)*H, (cx+w/2)*W, (cy+h/2)*H]})\n"
    "    return len(nms(dets, 0.5))\n"
