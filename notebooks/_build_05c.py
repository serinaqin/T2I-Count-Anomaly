...
c.append(nbf.v4.new_code_cell(
    ...
    "# direction that decodes to neutral gray. NOTE: SDXL's VAE NaNs in fp16, so\n"
    "# we upcast it to float32 for the probe, then restore.\n"
    "@torch.no_grad()\n"
    ...
    "            rgb = vae.decode(z / sf).sample.mean(dim=(0, 2, 3))\n"
    "            cols.append((rgb - rgb0) / probe)\n"
    "        M = torch.stack(cols).cpu()                   # (4 channels, 3 RGB)\n"
    "    finally:\n"
    "        vae.to(orig)\n"
    "    if not torch.isfinite(M).all():\n"
    "        print('WARN: VAE probe non-finite; falling back to uniform bump')\n"
    "        return torch.ones(4) * raw['gray_amp']\n"
