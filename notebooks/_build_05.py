"""Generate notebooks/05_noise_injection.ipynb (Phase 5 - mitigation).

Count-aware noise injection at the initial latent: partition it into N boxes and
perturb the in-box noise so SDXL realizes N objects. Measures exact-count
accuracy vs baseline. Causally motivated by Phases 2c/4/4b (count is noise-bound,
set spatially early). Thin Colab driver.

Run: python notebooks/_build_05.py
"""
...
c.append(nbf.v4.new_markdown_cell(
    ...
    "We proved the count is bound to the initial noise and set spatially in the "
    "early window. So we mitigate THERE: split the initial latent into N "
    "non-overlapping boxes and perturb the in-box noise (Uniform-Scaled / Fixed "
    "/ Gaussian), then measure exact-count accuracy vs baseline.\n\n"
    "**Runtime:** GPU (~20 min)."))
...
c.append(nbf.v4.new_markdown_cell(
    "## How to read this\n"
    "- **A scheme with clearly higher exact-count accuracy than baseline** = a "
    "working, **training-free** mitigation at the level we proved matters (the "
    "noise). That closes the arc: interpret -> localize -> prove causal -> "
    "characterize (spatial, noise-bound) -> **mitigate**. The eyeball should show "
    "the injected images hitting the requested count in coherent scenes, "
    "especially at higher counts where baseline collapses.\n"
    "- **No gain** = the SDXL noise->count coupling needs a stronger layout prior "
    "(bigger boxes / perturbation, or the fine-tuning the paper pairs with it, "
    "which is out of scope). Report honestly; the causal *finding* stands "
    "regardless.\n"
    "- Watch image quality: over-strong injection can hurt coherence (a "
    "quality/accuracy trade-off to note)."))
...
import os
out = os.path.join(os.path.dirname(__file__), "05_noise_injection.ipynb")
nbf.write(nb, out)
print("wrote", out)
