...
def count_aware_latent(base, n, scheme="gaussian", gamma=0.1, omega=0.3,
    ...
    boxes and perturb the in-box noise. Schemes: uniform_scaled (scale by
    gamma), fixed (set to fixed_value), gaussian (add Gaussian bumps at box
    centers). `base` is (1, C, H, W); returns a modified copy."""
    lat = base.clone()
    _, _, H, W = lat.shape
